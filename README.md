# Genesis-Gnosis: мульти-агентная система
...
**Genesis-Gnosis** — это автономная мульти-агентная система, спроектированная для выполнения комплексных исследовательских задач и синтеза структурированных аналитических документов. Она воспроизводит когнитивный процесс эксперта: от декомпозиции задачи и сбора данных до написания, рецензирования и финальной верстки.

Система решает фундаментальные проблемы современных LLM — ограниченность контекстного окна, склонность к искажению фактов и отсутствие долгосрочной памяти — путем внедрения **гибридной модели памяти (Graph RAG)**, **состязательного (adversarial) цикла верификации** и **отказоустойчивой архитектуры**, поддерживающей возобновление работы после сбоев.
...
*   **🤖 Система агентов:** 7+ агентов, каждый с уникальной ролью (Архитектор, Исследователь, Писатель, Критик), работающих в оркестрованной последовательности.
...
##  Mission Control Center: Тактический Дашборд

Управление системой осуществляется через GUI-дашборд, обеспечивающий наблюдаемость процессов.

![Dashboard Interface](assets/screenshot.png)

*   **HyperCortex Visualizer:** 3D-проекция когнитивной активности в виде вращающегося икосаэдра.
...
| Агент | Роль | Задача и Техническая Реализация |
| :--- | :--- | :--- |
| 🏛️ **The Architect** | Стратегический Планировщик | **Основная Директива:** Декомпозиция неопределенности в строгую иерархию.<br><br>**Методология:** Агент выполняет "когнитивное сканирование" темы, симулируя внутренний монолог для выявления скрытых противоречий. Использует Chain-of-Thought (CoT) для генерации дерева подзадач. Результат преобразуется в валидируемый JSON-объект `ArticleMasterPlan` (через Pydantic), содержащий тезисы, цели и семантические связи для каждой главы. Поддерживает режимы самокоррекции при ошибках парсинга структуры. |
| 🕵️ **The Researcher**| Специалист по OSINT | **Основная Директива:** Агрегация данных с нулевой толерантностью к галлюцинациям.<br><br>**Методология:** Анализирует план Архитектора и генерирует уникальные поисковые запросы для каждой секции. Использует кастомный веб-драйвер с ротацией User-Agent для обхода защиты от ботов. Применяет алгоритм `EVIDENCE_DISTILLER`: загружает HTML, очищает DOM от шума (реклама, навигация) и извлекает только именованные сущности, статистику и цитаты. Найденные факты инъектируются в Граф Знаний (NetworkX) с привязкой к источнику. |
| ✍️ **The Writer**| Синтезатор Нарратива | **Основная Директива:** Преобразование структурированных данных в связный текст.<br><br>**Методология:** Использует механизм RAG (Retrieval-Augmented Generation): перед написанием текста опрашивает Граф Знаний для получения контекста. Управляет "скользящим окном" контекста (`ContextManager`), чтобы помнить сюжетную линию. При обнаружении массивов данных вызывает **Code Interpreter**: генерирует Python-код на лету, исполняет его в изолированном процессе для построения графиков (Matplotlib/Seaborn) и бесшовно встраивает их в документ. |
| ⚖️ **The Critic** | Adversarial-Рецензент | **Основная Директива:** Итеративное повышение качества через критику.<br><br>**Методология:** Действует как фильтр качества (Quality Gate). Получает черновик от Писателя и оценивает его по многомерной матрице: фактологическая точность, логическая связность, стиль. Возвращает объект `ReviewFeedback` с булевым флагом одобрения и списком конкретных требований (`required_edits`). Блокирует переход к следующей главе, пока текст не наберет проходной балл (score > 7/10). |
| 🌉 **Continuity Director**| Режиссер Повествования | **Основная Директива:** Обеспечение глобальной когерентности документа.<br><br>**Методология:** Решает проблему "фрагментированности", свойственную LLM. Анализирует векторное и смысловое окончание предыдущей главы и начало текущей. Генерирует "транзитные параграфы" (bridge-text), сглаживая переходы между темами. Гарантирует, что итоговый документ читается как единая монография, а не набор разрозненных эссе. |
| 👻 **Style Polymath**| Стилист и "Призрак" | **Основная Директива:** Мимикрия под естественный человеческий интеллект.<br><br>**Методология:** Финальный пост-процессинг. Проводит лингвистический анализ текста, выявляя паттерны машинной генерации (повторы, канцеляризмы, неестественная структура предложений). Переписывает текст, повышая лексическое разнообразие (Perplexity) и добавляя идиоматические обороты, свойственные экспертам в предметной области. |
...
##  Инженерные решения для Отказоустойчивости
...
##  Правовая Оговорка и Условия Использования (Disclaimer)
